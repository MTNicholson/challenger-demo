    ...
    <main className="space-y-6">
      ...
      <Card className="p-5">
        <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr] lg:items-center">
          <div>
            <div className="text-xs font-extrabold uppercase tracking-[0.14em] text-blue-700">Профиль бренда</div>
      ...
      <Link href={routes.brand.analytics} className="brand-glass brand-interactive group block rounded-2xl p-5">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div><div className="flex items-center gap-2 text-sm font-bold text-blue-700"><BarChart3 className="h-4 w-4" />Аналитика за неделю</div><h2 className="mt-2 text-2xl font-black text-slate-950">Челленджи увеличили визиты на {companyAnalytics.visitsGrowthPercent}%</h2></div>
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
            <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm"><Repeat2 className="h-4 w-4 text-blue-600"/><div className="mt-2 text-xl font-extrabold">{companyAnalytics.repeatVisitsPercent}%</div><div className="text-xs text-slate-500">повторных визитов</div></div>
            <div className="rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm"><Gift className="h-4 w-4 text-blue-600"/><div className="mt-2 text-xl font-extrabold">{companyAnalytics.issuedRewards}</div><div className="text-xs text-slate-500">наград выдано</div></div>
            <div className="hidden rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm sm:block"><TrendingUp className="h-4 w-4 text-blue-600"/><div className="mt-2 text-xl font-extrabold">{companyAnalytics.rewardConversionPercent}%</div><div className="text-xs text-slate-500">конверсия</div></div>
          </div>
          <div className="inline-flex items-center gap-2 font-black text-blue-700">Подробнее <ArrowRight className="h-4 w-4 transition group-hover:translate-x-1"/></div>
        </div>
      </Link>
      ...
      <Card className="p-5">
        ...
        <div className="mt-5 grid gap-3 lg:grid-cols-2">
          {brandChallenges.filter((challenge) => challenge.isActive).map((challenge) => (
            <div key={challenge.id} className="brand-interactive rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
