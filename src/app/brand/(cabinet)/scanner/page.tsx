...
export default async function StaffScannerPage() {
  ...
  return (
    <main className="space-y-6">
      <header><div className="text-sm font-bold text-blue-700">{brandName} · режим сотрудника</div><h1 className="mt-1 text-3xl font-black tracking-tight">Сканер наград</h1><p className="mt-2 text-slate-500">Отсканируйте QR-код гостя или введите код вручную</p></header>
      <div className="grid gap-6 lg:grid-cols-[1fr_380px]">
        <section className="brand-glass-dark rounded-2xl p-6 text-white">
          <div className="grid min-h-[360px] place-items-center rounded-2xl border border-white/15 bg-white/[0.07] shadow-inner">
            <div className="text-center"><div className="relative mx-auto grid h-60 w-60 place-items-center rounded-2xl border-2 border-dashed border-white/30"><span className="absolute left-4 top-4 h-8 w-8 border-l-4 border-t-4 border-sky-200" /><span className="absolute bottom-4 right-4 h-8 w-8 border-b-4 border-r-4 border-sky-200" /><QrCode className="h-28 w-28 text-white/75" /><div className="absolute left-7 right-7 top-1/2 h-0.5 bg-sky-200 shadow-lg shadow-sky-200/40" /></div><p className="mt-5 text-sm text-white/50">Наведите рамку на QR-код · камера в демо не используется</p></div>
          </div>
          <div className="mt-5 rounded-xl bg-white/10 p-4"><label htmlFor="reward-code" className="flex items-center gap-2 text-sm font-bold text-white/70"><Keyboard className="h-4 w-4" />Код награды</label><input id="reward-code" defaultValue={scannerMockResult.rewardCode} placeholder="Например, CP-4829" className="mt-3 w-full rounded-xl border border-white/10 bg-white px-4 py-3 font-black tracking-[0.16em] text-slate-950 outline-none focus:ring-2 focus:ring-blue-300" /></div>
        ...
        <aside className="brand-glass self-start rounded-2xl p-6 lg:sticky lg:top-28">
          ...
          <div className="mt-6 flex items-center gap-4"><div className="grid h-14 w-14 place-items-center rounded-xl bg-blue-50"><Gift className="h-6 w-6 text-blue-700" /></div><div><h3 className="font-black">{scannerMockResult.rewardTitle}</h3><p className="text-sm text-slate-500">{scannerMockResult.rewardBrand}</p></div></div>
