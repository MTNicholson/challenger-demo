  ...
  return (
    <main className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[430px_minmax(0,1fr)]">
        <section aria-label="Превью челленджа на телефоне" className="mx-auto w-full max-w-[430px] self-start rounded-[34px] border-[9px] border-slate-950 bg-slate-950 p-2 shadow-2xl shadow-slate-900/15">
          <div className="overflow-hidden rounded-[24px] bg-[#f7f9fc]">
            ...
            <div className="p-5">
              ...
              <article className="mt-5 rounded-2xl bg-slate-950 p-5 text-white shadow-xl">
                <div className="flex items-start justify-between"><span className="rounded-full bg-blue-400/15 px-3 py-1 text-xs font-black text-sky-200">Легко · 7 дней</span><span className="text-4xl">{campaign.emoji}</span></div>
                ...
                <div className="mt-5 space-y-2"><div className="flex items-center gap-3 rounded-xl bg-white/10 p-3 text-sm font-bold"><Target className="h-5 w-5 text-sky-200" />5 визитов за 7 дней</div><div className="flex items-center gap-3 rounded-xl bg-white/10 p-3 text-sm font-bold"><Gift className="h-5 w-5 text-amber-300" />Напиток + 200 монет</div></div>
              </article>
              <div className="mt-4 rounded-2xl bg-white p-4 shadow-sm"><div className="flex justify-between text-sm"><span className="font-black">Ваш прогресс</span><span className="font-bold text-blue-700">3 из 5</span></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full w-3/5 rounded-full bg-blue-600" /></div><p className="mt-3 text-xs leading-5 text-slate-500">Ещё 2 визита — и награда появится в кошельке.</p></div>
              <div className="mt-4 flex items-center gap-3 rounded-2xl bg-blue-50 p-4 text-blue-950"><MapPin className="h-5 w-5 shrink-0" /><div><div className="text-sm font-black">Ближайшая точка · 450 м</div><div className="mt-0.5 text-xs text-blue-800">Coffee Place, Невский 24</div></div></div>
            </div>
          </div>
        </section>

        <section className="space-y-5 self-start">
          <div className="rounded-2xl bg-white p-6 shadow-sm"><div className="text-sm font-semibold text-slate-400">Сводка для бренда</div><h2 className="mt-1 text-2xl font-black">Кофейный маршрут</h2><div className="mt-5 grid gap-3 md:grid-cols-2">{summary.map(({ icon: Icon, label, value }) => <div key={label} className="rounded-xl bg-slate-50 p-4"><Icon className="h-5 w-5 text-slate-400" /><div className="mt-3 text-xs font-black uppercase tracking-wider text-slate-400">{label}</div><div className="mt-1 text-sm font-black">{value}</div></div>)}</div></div>
          <div className="rounded-2xl bg-slate-950 p-6 text-white shadow-xl shadow-slate-900/10"><div className="flex items-center gap-2 text-sm font-semibold text-sky-200"><TrendingUp className="h-4 w-4" />Ожидаемый KPI</div><div className="mt-4 grid gap-3 sm:grid-cols-3"><div><div className="text-3xl font-black">600+</div><div className="mt-1 text-xs text-white/50">участников</div></div><div><div className="text-3xl font-black">35%</div><div className="mt-1 text-xs text-white/50">завершат маршрут</div></div><div><div className="text-3xl font-black">210</div><div className="mt-1 text-xs text-white/50">активаций награды</div></div></div><p className="mt-5 border-t border-white/10 pt-4 text-xs leading-5 text-white/45">Оценка основана на текущей аудитории бренда: {companyAnalytics.followers.toLocaleString("ru-RU")} подписчиков и {companyAnalytics.repeatVisitsPercent}% повторных визитов.</p></div>
          <div className="flex flex-wrap gap-3 rounded-2xl bg-white p-4 shadow-sm"><Link href={routes.brand.analytics} className={buttonClasses({ variant: "dark" })}><BarChart3 className="h-4 w-4" />К аналитике</Link><Link href={routes.brand.challenges} className={buttonClasses({ variant: "ghost" })}>К списку челленджей</Link></div>
        </section>
      </div>
    </main>
  );
