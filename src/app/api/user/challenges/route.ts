...
export async function GET() {
  ...
  const challenges = await prisma.userChallenge.findMany({
    ...
    include: { reward: { select: { status: true } } },
  });
...
export async function POST(request: Request) {
  ...
  const participation = await prisma.userChallenge.upsert({ where: { userId_challengeId: { userId: user.id, challengeId: challenge.id } }, update: {}, create: { userId: user.id, challengeId: challenge.id, progressTotal: Math.max(1, body.progressTotal ?? 1) } });
  await prisma.userChallengeEvent.create({ data: { userId: user.id, challengeId: challenge.id, participationId: participation.id, type: "challenge_activated", metadata: { source: "user" } } }).catch(() => undefined);
