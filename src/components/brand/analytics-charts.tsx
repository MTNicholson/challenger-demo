  ...
  return (
    <ResponsiveContainer width="100%" height="100%" initialDimension={chartSize}>
      <AreaChart data={weeklyActivity} margin={{ top: 8, right: 4, left: -24, bottom: 0 }}>
        <defs><linearGradient id="visits" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.32}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient></defs>
        ...
        <Area type="monotone" dataKey="visits" stroke="#10b981" strokeWidth={3} fill="url(#visits)"/>
      </AreaChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height="100%" initialDimension={chartSize}>
      <BarChart data={locationPerformance} layout="vertical" margin={{ top: 0, right: 12, left: 12, bottom: 0 }}>
        ...
        <Tooltip contentStyle={tooltipStyle} formatter={(value) => [value, "Визитов"]}/>
        <Bar dataKey="visits" radius={[0, 8, 8, 0]}>{locationPerformance.map((item, index) => <Cell key={item.id} fill={index === 0 ? "#10b981" : "#cbd5e1"}/>)}</Bar>
      </BarChart>
    </ResponsiveContainer>
  );
}
  ...
  return (
    <ResponsiveContainer width="100%" height="100%" initialDimension={chartSize}>
      <BarChart data={challengePerformance} margin={{ top: 8, right: 4, left: -24, bottom: 0 }}>
        ...
        <Bar dataKey="completed" name="Завершили" fill="#10b981" radius={[6, 6, 2, 2]}/>
      </BarChart>
    </ResponsiveContainer>
  );
}
