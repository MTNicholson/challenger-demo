...
## Публичный статус бренда

Модерация и публичная видимость разделены: `status=approved` даёт бренду доступ к кабинету, а `publicStatus=ONLINE` делает его видимым пользователям. Новый бренд создаётся скрытым (`OFFLINE`). После подтверждения платформой владелец запускает или скрывает бренд в `/brand/settings`; при скрытии кабинет бренда и кабинеты точек продолжают работать.

## Кабинет точки

### Второй этап: локальные награды и челленджи

В режимах `Extended` и `Flagship` администратор точки может создавать локальные награды. `Extended` отправляет новый челлендж как заявку в основной кабинет бренда, где её можно подтвердить, отклонить или вернуть на доработку. `Flagship` публикует челлендж сразу, но только для своей точки. `Standard` и сотрудники точки не получают доступа к этим разделам.

В основном кабинете бренда откройте `/brand/locations` и создайте новую точку: для неё обязательны режим (`Standard`, `Extended` или `Flagship`) и данные администратора точки. Администратор и сотрудники входят на `/brand/auth/login` через вкладку «Кабинет точки».

- `Standard` — аналитика точки, сотрудники, сканер и лог.
- `Extended` — те же функции плюс подготовленные разделы челленджей и наград.
- `Flagship` — то же, с пометкой автономного режима для будущего запуска челленджей.

Администратор точки управляет только сотрудниками своей точки. Сотрудник точки видит только `/location/scanner`, `/location/scan-log` и выход. QR-сканер, скан-лог, локальные награды и запуск челленджей на этом этапе являются UI-заглушками.
...
Админка доступна по адресу `/adminconsole`. Регистрации администраторов нет: задайте `ADMIN_EMAIL`, bcrypt-хеш в `ADMIN_PASSWORD_HASH` и `ADMIN_SESSION_SECRET` длиной не менее 32 символов. Хеш можно получить командой:

```bash
npm run admin:hash -- "надёжный-пароль"
```

Перед production примените миграции `npm run prisma:migrate:deploy`. Все `/api/admin/*` проверяют отдельную httpOnly-сессию администратора, а изменения брендов и ручные завершения участия попадают в `/adminconsole/audit`.
